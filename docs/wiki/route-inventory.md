...
| 구분 | 개수 | 상태 |
|------|------|------|
| 공개 페이지 (`pages/**`, `/p/` 제외) | 134 | ✅ 121×200 + 10×3xx(의도된 리다이렉트) |
| 비공개 페이지 (`/p/`, `/en/p/`) | 65 | ✅ 전부 200 |
| 정적 자산 (`public/**/*.html|pdf`) | 16 | ✅ 전부 200 |
| **합계** | **215** | ✅ 유실 0건 |
...
| `/webinar/*` | 3 | 하이테커 웨비나: 허브 `/webinar` + `/webinar/prep`(사전 준비) + `/webinar/handbook`(참가자 핸드북). 메뉴 숨김, 공개 접근. 정적 파일 `public/files/webinar/` (2026-09-21) | 코드몬/Claude Code |
